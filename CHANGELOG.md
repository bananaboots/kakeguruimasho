...
- **Desktop adaptation.** Three-pane salon chassis at `>=1024px`: persistent left rail (house mark + nav + brand), center stage (the active route), right rail (Bonus widget + Pot + Recent Pulls). Mobile shell unchanged below `1024px`.
  ...
  - **Cinematic Onboarding shell.** `<CinematicShell>` wraps the Onboarding flow at desktop with a full-bleed felt + paper-grain backdrop and four corner ornate motifs framing a centered `OrnateFrame` card. No-op at mobile.
  - **Hover affordances** under `@media (hover: hover) and (min-width: 1024px)` on `BrassButton`, `RitualCard`, `Chip`, Vault tier rows, and the lever cursor. Touch devices and mobile viewports never see these rules.
  - **`useIsDesktop` hook** + shared `navItems.ts` config so `BottomNav` (mobile) and `LeftRail` (desktop) render identical items. `BonusTimerBanner` only mounts at mobile -- desktop's right rail surfaces the same state via the new `useBonusTimerSummary` hook.
  - **Tests.** New `desktop-chromium` Playwright project at `1440x900`; `e2e/desktop-chassis.spec.ts` asserts rail visibility, bottom-nav hidden, Vault grid columns, and cinematic Onboarding wrap. `Drawer` and `CinematicShell` get unit tests; `useIsDesktop` is covered.
